...
const Actions: React.FC<Props> = ({
  ...
  useEffect(() => {
    setShareLink(`${window.location.origin}/dhaaga/${threadId.toString()}`);
  }, []);
    ...
    <>
      {profile ? (
        <div className="flex gap-3">
          <p className="text-small-medium text-light-3">Likes: {likesCount}</p>
          {isCommnet && (
            <>
              <Separator
              ...
              />
              <Link href={`/dhaaga/${threadId}`}>
                <p className="text-small-medium text-light-3">View Primary</p>
              </Link>
            </>
          )}
        </div>
